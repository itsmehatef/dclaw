...
3. **Seccomp profile.** Rely on Docker's daemon-loaded default profile (applied automatically when no `seccomp=` SecurityOpt is set). The original plan called for an explicit `SecurityOpt: "seccomp=default"` pin, but Docker rejects that literal — see §11 Q2 for the syntax constraint (only `unconfined` or JSON content are accepted after `seccomp=`). A tighter custom policy is deferred.
...
- **Custom (tighter-than-default) seccomp profile.** beta.2 relies on Docker's daemon-loaded default profile (explicit pin via `seccomp=default` is not supported by Docker — see §11 Q2). A tighter policy denying `keyctl`, `add_key`, `personality`, `ptrace` (already denied by default for non-root, but belt-and-suspenders) shipped as a JSON profile file and applied via `SecurityOpt: "seccomp=/path/to/profile.json"` is a follow-up PR. See §11 Q2.
...
**Goal:** Drop all Linux capabilities, block setuid/setgid privilege escalation, rely on Docker's daemon-loaded default seccomp profile, cap PIDs. Zero user-visible change; the agent still runs. Attack surface shrinks by ~80% of the classic container-escape CVE corpus.

**Note on seccomp pinning (hotfix `v0.3.0-beta.2-sandbox-hardening.1`):** The original PR-A spec called for an explicit `SecurityOpt: "seccomp=default"` entry. That token is invalid — Docker parses the value after `seccomp=` as either `unconfined` or JSON profile content, never a profile-name selector, and returns "Decoding seccomp profile failed: invalid character 'd' looking for beginning of value" when it sees the literal `default`. The fix is to not set any `seccomp=` SecurityOpt: Docker's daemon-loaded default profile is applied automatically under that condition, so the threat-model coverage (§6.3 keyctl, ptrace denial) is preserved. The spec rows below reflect the corrected shape.

**Files changed:**

| File | Kind | Notes |
|---|---|---|
| `/Users/macmini/workspace/agents/atlas/dclaw/internal/sandbox/docker.go` | MODIFIED | `CreateAgent` (lines 90-136): add four fields to `HostConfig`. (1) `CapDrop: []string{"ALL"}`. (2) `SecurityOpt: []string{"no-new-privileges:true"}` — Docker's built-in default seccomp profile applies automatically when no `seccomp=` SecurityOpt is set (an explicit `seccomp=default` literal is rejected as invalid JSON). (3) `Resources: container.Resources{PidsLimit: pidsLimitPtr(256)}`. (4) New package-level helper `func pidsLimitPtr(n int64) *int64`. Net lines: ~+25. |
| `/Users/macmini/workspace/agents/atlas/dclaw/internal/sandbox/docker.go` | MODIFIED | New package-level constants at the top of the file (after the import block, before `ErrDockerFailure`): `// Container posture constants (beta.2-sandbox-hardening). Held as package-level named constants so tests can assert the exact shape without string-matching the implementation.` Define `DefaultCapDrop = []string{"ALL"}`, `DefaultSecurityOpt = []string{"no-new-privileges:true"}`, `DefaultPidsLimit int64 = 256`. Use these inside `CreateAgent` so the test harness can reference them. Net lines: ~+10. |
| `/Users/macmini/workspace/agents/atlas/dclaw/internal/sandbox/docker_test.go` | NEW | New file. Defines an in-package test harness `type captureClient struct { lastCfg *container.Config; lastHostCfg *container.HostConfig }` plus a stubbed Docker API client interface the tests can inject. Test function `TestCreateAgentAppliesBeta2HardeningPosture` asserts after a `CreateAgent` call: `assert.Equal(t, []string{"ALL"}, gotHostCfg.CapDrop)`, `assert.Contains(t, gotHostCfg.SecurityOpt, "no-new-privileges:true")`, `assert.NotContains(t, gotHostCfg.SecurityOpt, "seccomp=default")` (regression guard against the invalid literal), `require.NotNil(t, gotHostCfg.Resources.PidsLimit)`, `assert.Equal(t, int64(256), *gotHostCfg.Resources.PidsLimit)`. Uses `testing/quick`-style table for the happy path + negative path (empty spec.Workspace should still apply all four). Net lines: ~+180. |
...
# Docker's default seccomp profile is applied automatically by the daemon;
# this test confirms it's active (not our explicit pin — we don't pin
# because `seccomp=default` is not a valid Docker SecurityOpt token, only
# `unconfined` or JSON are accepted). The default profile denies unshare
# with CLONE_NEWUSER for unprivileged tasks. This test exercises one
# concrete denied syscall; a full profile regression suite lives in
# `syscall_blocklist_test.sh` under follow-ups.
...
**Post-beta.2 mitigation:** Docker's daemon-loaded default seccomp profile (applied automatically when no `seccomp=` SecurityOpt is set — see §4.1 hotfix note) blocks `ptrace` for non-capability workloads. Test 23 probe #3 (`unshare -U`) confirms the default profile is active; `ptrace` follows the same profile. Additionally, Yama LSM on the host side would block `ptrace` of non-parent processes (ptrace_scope=1), but that is a host-side setting.
...
**Post-beta.2 mitigation:** PR-A relies on Docker's daemon-loaded default seccomp profile, which denies `keyctl`, `add_key`, and `request_key` for unprivileged containers. The original PR-A spec called for explicit `SecurityOpt: "seccomp=default"` but that literal is rejected by Docker (see §4.1 hotfix note); the profile is applied automatically whenever no `seccomp=` SecurityOpt is set, so the denial still holds. Custom-profile extension (deny all three syscalls explicitly via a JSON profile) is §11 Q2 follow-up — a bespoke JSON file IS a valid `seccomp=` value.
...
**Decided: rely on Docker's daemon-loaded default in beta.2, tighter custom as follow-up.** Docker's default seccomp profile blocks ~70 syscalls that ordinary workloads never need (`keyctl`, `add_key`, `request_key`, `userfaultfd`, `bpf`, `personality`, etc.). Shipping the default is a full order-of-magnitude improvement over what we have today.

**Hotfix note (`v0.3.0-beta.2-sandbox-hardening.1`):** The original beta.2 decision was "pin default explicitly" via `SecurityOpt: "seccomp=default"`, on the theory that the explicit pin would defend against a daemon whose global config had disabled default seccomp. That theory was wrong on the SecurityOpt syntax: Docker parses the value after `seccomp=` as `unconfined` or JSON content, never a profile-name selector, and rejects the literal `default` with "Decoding seccomp profile failed: invalid character 'd' looking for beginning of value". Beta.2 therefore relies on the daemon-loaded default profile, which Docker applies automatically whenever no `seccomp=` SecurityOpt is set. Operators who have disabled default seccomp daemon-side need to either re-enable it or run with a custom profile (see follow-up below) — there is no Docker-supported way to force the default from the client side.

A tighter custom profile denying additional syscalls that pi-mono specifically doesn't need (e.g., `ptrace` even for same-user, `mount`, `umount2`, `pivot_root`, `clone3` with non-standard flags) is a valuable follow-up but adds ~200 lines of JSON to author and maintain. beta.2 leaves this out. The custom-profile path IS expressible through `SecurityOpt: "seccomp=/path/to/profile.json"` (JSON content is the accepted value after `seccomp=`, unlike a profile-name keyword).

**Residual risk:** Default seccomp's allowlist grows as the kernel adds syscalls. New primitives that land in Linux 6.x+ may not be explicitly denied. Operators who have disabled default seccomp daemon-side silently lose the coverage until the custom-profile follow-up lands. Mitigation: periodic audit against CIS Docker Benchmark (roughly annual cadence); document the daemon-default dependency in `docs/workspace-root.md`.
